...
class MqttController(val context: Context) : IMqttController {
    ...
    override fun createMqttClient(
        context: Context,
        brokerURL: String,
        clientId: String,
        deviceSymmetricKey: ByteArray
    ): MqttAndroidClient? {
        return try {
                    ...
                    try {
                        ...
                        CoroutineScope(Dispatchers.IO).launch {
                            deviceSymmetricKey.let { key ->
                                try {
                                    val symmetricKey =
                                        JvmAesService.loadKey(AesAlgorithm.CbcWithPkcs7Padding, key)
                                    val decrypted = JvmAesService.decrypt(decodedData, symmetricKey)
                                    val sensorData =
                                        mapper.readValue(decrypted, SensorDataContent::class.java)
                                    println("decrypted $decrypted.to")
                                    _rtData.value = _rtData.value.copy(
                                        liveSensorData = _rtData.value.liveSensorData.toMutableList()
                                            .apply {
                                                add(sensorData)
                                            }
                                    )
